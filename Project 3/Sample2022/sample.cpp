...
Display( )
{
	...
	SetMaterial(1.0f, 0., 0., 0.);
	glCallList(catDL);
	glPopMatrix();

	glPushMatrix();
	glTranslatef(0., 0., -0.4f);
	SetMaterial(0.f, 1.f, 0., 64.);
	glCallList(bunnyDL);
	glPopMatrix();

	glPushMatrix();
	glTranslatef(0.4f, 0., 0.);
	SetMaterial(0.f, 0., 1.f, 128.);
	glCallList(duckDL);
	glPopMatrix();
	...
	if (lightType == 1) {
		SetPointLight(GL_LIGHT0, x, 0.9f, z, r, g, b);
	}
	else {
		SetSpotLight(GL_LIGHT0, x, 0.6f, z, 0.f, -1.f, 0., r, g, b);
	}
