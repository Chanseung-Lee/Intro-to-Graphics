...
main( int argc, char *argv[ ] )
{
	...
	Sc.Init();
	Sc.AddTimeValue(0.0, 0.6);
	Sc.AddTimeValue(3.0, 0.4);
	Sc.AddTimeValue(6.0, 0.6);
	Sc.AddTimeValue(9.0, 0.4);
	

	Tc.Init();
	Tc.AddTimeValue(0.0, 0.4);
	Tc.AddTimeValue(3.0, 0.6);
	Tc.AddTimeValue(6.0, 0.4);
	Tc.AddTimeValue(9.0, 0.6);
...
Display( )
{
	...
	float scale_factor_s = sin(Time);  // Adjust the amplitude and frequency as needed
	float scale_factor_t = sin(Time);
	...
	if (KeytimePatternOn)
	{
		sc = Sc.GetValue(nowSecs);
		tc = Tc.GetValue(nowSecs);
	}
	if (TimePatternOn)
	{
		sin_radius = sin(Time * 20) / 10;
		cos_radius = cos(Time * 20) / 10;
	}
	...
	Pattern.SetUniformVariable("uRs", sin_radius);
	Pattern.SetUniformVariable("uRt", cos_radius);
